...
const DUMMY_DOCS = [
  {
    id: 1,
    title: '발사르탄 니트로사민 유전독성 불순물 MV 보고서',
    type: 'MV',
    standard: 'MFDS',
    equipment: 'HPLC',
    pages: 45,
  ...
  {
    id: 2,
    title: '메트포르민 CTD 모듈 3 완성본 (ICH Q3A 기반)',
    type: 'CTD',
    standard: 'MFDS',
    equipment: 'LC-MS/MS',
    pages: 120,
  ...
  {
    id: 3,
    title: '고성능 UV 흡광도 별규시험법 SOP 표준 양식',
    type: 'SOP',
    standard: 'USP',
    equipment: 'UV-Vis',
    pages: 18,
  ...
  {
    id: 5,
    title: '아토르바스타틴 CV 적격성 평가 보고서 (IQ/OQ/PQ)',
    type: 'CV',
    standard: 'MFDS',
    equipment: 'HPLC',
    pages: 67,
  ...
  {
    id: 6,
    title: '아목시실린 원료 CTD Module 2 품질총괄보고서',
    type: 'CTD',
    standard: 'MFDS',
    equipment: 'GC',
    pages: 88,
  ...
  {
    id: 7,
    title: 'GC-Headspace 잔류용매 시험법 MV 완성 패키지',
    type: 'MV',
    standard: 'USP',
    equipment: 'GC',
    pages: 53,
    ...
    verified: true,
    isNew: false,
    tags: ['잔류용매', 'ICH Q3C', 'Headspace'],
  },
  {
    id: 8,
    title: '니트로사민류 위험평가 SOP 및 체크리스트',
    type: 'SOP',
    standard: 'MFDS',
    equipment: 'LC-MS/MS',
    pages: 24,
    ...
    verified: true,
    isNew: true,
    tags: ['니트로사민', '위험평가', 'NDSRI'],
  },
  {
    id: 9,
    title: 'USP <1> 주사제 입자 적격성 평가 프로토콜',
    type: 'Protocol',
    standard: 'USP',
    equipment: 'Light Obscuration',
    pages: 29,
    price: 900000,
    verified: false,
    isNew: false,
  ...
  {
    id: 10,
    title: '레보티록신 함량 균일성 MV (EP 2.9.40 기반)',
    type: 'MV',
    standard: 'EP',
    equipment: 'HPLC',
    pages: 41,
...
export default function LicenseMarketPage() {
  ...
  }, [search, selectedTypes, selectedStandards, maxPrice])
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {HOT_TAGS.map(tag => (
              <button
                key={tag}
                onClick={() => setSearch(tag.replace('#', '').replace('_', ' '))}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex gap-6">
        ...
        <aside className="w-56 shrink-0 space-y-6">
              ...
              {DOC_TYPES.map(t => (
                <label key={t} className="flex items-center gap-2.5 cursor-pointer group">
                  ...
                  <span className="ml-auto text-xs text-gray-400">
                    {DUMMY_DOCS.filter(d => d.type === t).length}
                  </span>
                </label>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {STANDARDS.map(s => (
                <label key={s} className="flex items-center gap-2.5 cursor-pointer group">
                  ...
                  <span className="ml-auto text-xs text-gray-400">
                    {DUMMY_DOCS.filter(d => d.standard === s).length}
                  </span>
                </label>
              ))}
            </div>
          ...
          {(selectedTypes.length > 0 || selectedStandards.length > 0 || maxPrice < 6000000) && (
            <button
              onClick={() => { setSelectedTypes([]); setSelectedStandards([]); setMaxPrice(6000000) }}
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-4">
            ...
            <select className="text-xs border border-gray-200 rounded-lg px-3 py-1.5 text-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white">
              <option>최신순</option>
...
function DocCard({ doc }: { doc: typeof DUMMY_DOCS[0] }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5 flex flex-col gap-3 hover:shadow-md hover:border-blue-200 transition-all">
      {/* 뱃지 */}
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {doc.isNew && (
          <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-green-500 text-white">
            NEW
          </span>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1">
        {doc.tags.map(tag => (
          <span key={tag} className="text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
            #{tag}
          </span>
        ))}
      </div>
